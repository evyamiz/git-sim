...
def main():
    ...
    parser.add_argument("--title", help="Custom title to display at the beginning of the animation", type=str,
                        default="Git Sim, by initialcommit.com")
    parser.add_argument("--logo", help="The path to a custom logo to use in the animation intro/outro", type=str,
                        default=os.path.join(str(pathlib.Path(__file__).parent.resolve()), "logo.png"))
    parser.add_argument("--outro-top-text", help="Custom text to display above the logo during the outro", type=str,
                        default="Thanks for using Initial Commit!")
    parser.add_argument("--outro-bottom-text", help="Custom text to display below the logo during the outro", type=str,
    ...
    parser.add_argument("--media-dir", help="The path to output the animation data and video file", type=str,
                        default=".")
    parser.add_argument("--low-quality", help="Render output video in low quality, useful for faster testing",
                        action="store_true")
    ...
    parser.add_argument("--max-branches-per-commit", help="Maximum number of branch labels to display for each commit",
                        type=int, default=1)
    parser.add_argument("--max-tags-per-commit", help="Maximum number of tags to display for each commit", type=int,
                        default=1)
    parser.add_argument("-d", "--disable-auto-open",
    ...
    log.add_argument("--commits", help="The number of commits to display in the simulated log output", type=int,
                     default=5, choices=range(1, 13))
    ...
    reset.add_argument("commit", nargs="?", help="The ref (branch/tag), or commit ID to simulate reset to", type=str,
                       default="HEAD")
    ...
    revert.add_argument("commit", nargs="?", help="The ref (branch/tag), or commit ID to simulate revert", type=str,
                        default="HEAD")

    merge = subparsers.add_parser("merge", help="merge -h")
    merge.add_argument("branch", nargs=1, type=str,
                       help="The name of the branch to merge into the active checked-out branch")
    merge.add_argument("--no-ff",
    ...
    cherrypick.add_argument("commit", nargs=1, type=str,
